...
const RegularCollector = require(__dirname + '/regular.js');
...
function Planner() {
  ...
  this.regularTasks = new RegularCollector();
  this.priorityQueue = [];
  this.firstWeekStart = new Date(2019, 1, 7);
}
...
Planner.prototype.add = function(task) {
  if (task.type === 'single') {
    ...
    if (task.weekPlan) {
      this.regularTasks.add(task.weekPlan);
    }
  }
};
